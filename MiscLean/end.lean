/-
What are "ends" and "coends"? Given a functor F: C^op x C → D, nLab says:
"An end is an object e of X equipped with a universal extranatural transformation from e to F"
Otoh Wikipedia says:
"An end is a universal dinatural transformation from an object e of X to F"
So what are extranatural and dinatural? Ugh... 
Let's juts use this blog post:
...
structure wedge {C D: Cat} (e: D.obj) (F: functor (Product (opposite C) C) D) where
...
structure end2 {C D: Cat} (e: D.obj) (F: functor (Product (opposite C) C) D) where
  w: wedge e F
  universal_exists:
    ∀ e': D.obj,
    ∀ w': wedge e' F,
