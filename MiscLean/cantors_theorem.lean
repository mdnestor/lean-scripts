...
Let X be a set.
Any function from X to the power set of X is non-surjective.

Proof:

Let X be a set, and let P(X) be its power set.
Then every function f: X → P(X) is non-surjective.
...
This theorem is in located in Mathlib under Mathlib.Logic.Function.Basic.lean as cantor_surjective
https://github.com/leanprover-community/mathlib4/blob/master/Mathlib/Logic/Function/Basic.lean
